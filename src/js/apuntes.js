...
export function createScore(){
        ...
        score.alt = "Marcador"
        score.className = CLASSES.UX_SCORE
        score.textContent = "Marcador"
        ELEMENTS.push(score)
        div.appendChild(score)
    }

    export function nextTurn(){
        let turn = document.createElement('h2')
        turn.alt = "Turno"
        turn.className = CLASSES.UX_TURN
        turn.textContent = "Le toca jugar a:"
        ELEMENTS.push(turn)
    ...
    for (let p = 0; p < players.length; p++) {
        if (players[p].homePieces == players[p]._keepPieces){
console.log(`${players[p]._color} a terminado la partida`)
        }else{
            scoreKeep.push(players[p])
console.log(`${players[p]._color} no a terminado la partida`)
        }
    }
    return score
}
